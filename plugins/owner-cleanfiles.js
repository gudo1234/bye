...
const _0xcleanTempFiles = async () => {
    ...
    if (_0xallTempFiles.length === 0) {
        return `${e} No se encontraron archivos temporales para eliminar.`;
    }
    ...
    return `${e} Se eliminaron ${_0xallTempFiles.length} archivos temporales correctamente.`;
};

let _0xhandler = async (m, { conn, isOwner }) => {
    if (!isOwner) return conn.reply(m.chat, `${e} Solo el propietario puede usar este comando.`, m);

    try {
        const _0xresult = await _0xcleanTempFiles();
        await conn.reply(m.chat, _0xresult, m);
    } catch (_0xerror) {
        await conn.reply(m.chat, `${e} Error: ${_0xerror.message}`, m);
    }
};

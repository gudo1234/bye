...
if (!text) return conn.reply(m.chat, `${e} Por favor, ingresa un enlace de Twitter/X valido.`, m, rcanal);
if (enviando) return;
    enviando = true;
try {
   m.react(rwait)
 ...
 if (res?.result.type == 'video') {
     m.react(done)
     const caption = res?.result.caption ? res.result.caption : `${e} Aqui tienes ฅ^•ﻌ•^ฅ.`;
     ...
     const caption =
    m.react(done)
    res?.result.caption ? res.result.caption : `${e} Aqui tienes ฅ^•ﻌ•^ฅ.`;
     for (let i = 0; i < res.result.media.length; i++) {
    ...
    m.react(error)
    enviando = false;
    return conn.reply(m.chat, `${e} Ocurrió un error.`, m, fake);
    return;
  }
};    

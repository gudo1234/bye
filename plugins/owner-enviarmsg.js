...
let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!text && !m.quoted) {
        return m.reply(`${e} *Por favor, escribe tu solicitud.*\n\n> *Elige una categoría:*\n\na). Sugerencia 💡\nb). Propuesta 📝\nc). Publicidad 📢\nd). Opinión 💬\ne). Pregunta 🚀\nf). Eventos 🎉\ng). Frases ✨\nh). Confesión anónima 🕵\n\n> 🌹 Ejemplo: ${usedPrefix + command} c Texto`);
    }
    ...
    if (!suggestionText) {
        return m.reply(`${e} Debes agregar un texto después de seleccionar la categoría.\nEjemplo: ${usedPrefix + command} a Mi solicitud es...`);
    }
    ...
    if (!category) {
        return m.reply(`${e} Opción inválida. Elige una categoría correcta: a, b, c o d.`);
    }

    m.reply(`${e} Tu Publicación ha sido enviada a los administradores para su revisión.`);
...
await conn.reply(sender, `${e} Solicitud aceptada, Grupo:\n${gp4}`);
delete suggestionQueue[suggestionId];
}};

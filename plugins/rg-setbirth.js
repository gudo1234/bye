...
if (user.birth) {
return conn.reply(m.chat, `${e} Ya tienes una fecha establecida, si quieres borrar la fecha actual usa:\n> » ${usedPrefix}delbirth`, m);
}

if (!text) return conn.reply(m.chat, `${e} Debes ingresar una fecha válida para tu cumpleaños.\n\n> ✐ Ejemplo » *${usedPrefix + command} 01/01/2000* (dia/mes/año)`, m);
...
if (!birth) {
return conn.reply(m.chat, `${e} Recuerda elegir una fecha de nacimiento válida.\n> Ejemplo: ${usedPrefix + command} 01/12/2024`, m);
}

user.birth = birth;
return conn.reply(m.chat, `${e} Se ha establecido tu fecha de nacimiento como: *${user.birth}*!`, m);
};

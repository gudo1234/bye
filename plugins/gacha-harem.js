...
async function loadCharacters() {
    try {
        ...
        throw new Error(`${e} No se pudo cargar el archivo characters.json.`);
    }
}
...
let handler = async (m, { conn, args }) => {
    try {
        ...
        if (userCharacters.length === 0) {
            await conn.reply(m.chat, `${e} No tiene personajes reclamados en tu harem.`, m);
            return;
        }
        ...
        if (page < 1 || page > totalPages) {
            await conn.reply(m.chat, `${e} Página no válida. Hay un total de *${totalPages}* páginas.`, m);
            return;
        }

        let message = `${e} Personajes reclamados ✿\n`;
        ...
        await conn.reply(m.chat, `${e} Error al cargar el harem: ${error.message}`, m);
    }
};

...
`.trim();
  ...
  const id = m.chat;
  if (id in global.math) return conn.reply(m.chat, `${e} Todavía hay un juego en proceso en este chat.`, global.math[id][0]);
  const math = genMath(mode);
  global.math[id] = [
    await conn.reply(m.chat, `${e} Cuanto es el resultado de ${math.str}?\n\n🕒 Tiempo: ${(math.time / 1000).toFixed(2)} segundos\n*🍬 Premio: ${math.bonus} XP*`, m),
    math, 4,
    setTimeout(() => {
      if (global.math[id]) {
        conn.reply(m.chat, `${e} Se a finalizado el tiempo para responder.\n\n🍬 La respuesta es ${math.result}`, m),

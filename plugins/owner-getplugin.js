...
const handler = async (m, { conn, isROwner, usedPrefix, command, text }) => {
  ...
  if (!text) {
    return conn.reply(m.chat, `${e} *Ingrese el nombre de algún plugin (archivo) existente*\n\n*—◉ Ejemplo*\n*◉ ${usedPrefix + command}* info-infobot\n\n*—◉ Lista de plugins (archivos) existentes:*\n*◉* ${ar1.map((v) => ' ' + v).join`\n*◉*`}`, m);
  }

  if (!ar1.includes(text)) {
    return conn.reply(m.chat, `${e} *No se encontró ningún plugin (archivo) llamado "${text}", ingrese alguno existente*\n\n*==================================*\n\n*—◉ Lista de plugins (archivos) existentes:*\n*◉* ${ar1.map((v) => ' ' + v).join`\n*◉*`}`, m);
  }

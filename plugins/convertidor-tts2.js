...
const handler = async (m, { conn, usedPrefix, command, text, args }) => {
  ...
  if (!efecto) {
    ...
    let responseText = `${e} *No haz ingresado un efecto, por favor ingresa un efecto de voz.*\n\n*Elige uno de los siguientes efectos:*\n`;
  ...
  if (!efectoValido) return conn.sendMessage(m.chat, { text: `${e} *El efecto proporcionado no existe en la lista, utiliza ${usedPrefix + command} para conocer la lista de efectos.*` }, { quoted: m });

  if (!texto) return conn.sendMessage(m.chat, {text: `${e} *Ingresa el texto que quieras convertir a audio.*\n\n*Ejemplo:*\n*◉ ${usedPrefix + command} ${efecto} Hola, este es un ejemplo de uso del comando.*`}, {quoted: m});

...
await conn.reply(m.chat, `${e} *@${m.sender.split("@")[0]} Registrado en el juego* ✅`, m, { mentions: [m.sender] })
}
...
fila = null, columna = null, sopaNube = null, sopaPalabra = null, sopaDir = null, userSP = null, cambioLetra = null
intentos = 0
await m.reply(`${e} *Agotaste los intentos!! la palabra _"${sopaPalabra}"_ Se encontraba en la dirección _${cambioLetra}_ De la fila _${fila}_ Y Columna _${columna}_*`)
return  
} else {
intentos -= 1
await m.reply(`${e} *Incorrecto. te quedan _${intentos}_ Intentos!!*${intentos === 1 ? '' : `\n*Palabra a encontrar:* \`\`\`${sopaPalabra}\`\`\``}\n\n${intentos === 1 ? `\`\`\`👾 Pista!!\`\`\`\n*La palabra _${sopaPalabra}_ Se encuentra en la dirrección _"${cambioLetra}"_*\n\n` : ''}${sopaNube}`)
return
}}
}}

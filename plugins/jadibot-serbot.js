...
if (new Date - global.db.data.users[m.sender].Subs < 120000) return conn.reply(m.chat, `${e} Debes esperar ${msToTime(time - new Date())} para volver a vincular un *Sub-Bot.*`, m)
if (Object.values(global.conns).length === 30) {
return m.reply(`${e} No se han encontrado espacios para *Sub-Bots* disponibles.`)
}
...
conn.reply(m.chat, `${e} Use correctamente el comando » ${usedPrefix + command} code`, m)
return
}

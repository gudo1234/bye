let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!conn.menfess || !conn.menfess[id]) {
        throw m.reply(`${e} *Error:* No se encontró ningún mensaje con el ID *${id}*.`);
    }
    
    let { dari, penerima } = conn.menfess[id];
    
    if (m.sender !== penerima) throw m.reply(`${e} No tienes permiso para responder a este mensaje.`);

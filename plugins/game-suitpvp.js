...
const handler = async (m, {conn, usedPrefix, text}) => {
  ...
  if (Object.values(conn.suit).find((room) => room.id.startsWith('suit') && [room.p, room.p2].includes(m.sender))) throw `${e} Termina tu partida antes de iniciar otra.`;
  
  const textquien = `${e} A quién quieres desafiar? etiqueta a un usuario.\n\n*—◉ Ejemplo:*\n${usedPrefix}suit @tag`;
  
  if (!userToChallenge) return m.reply(textquien, m.chat, {mentions: conn.parseMention(textquien)});
  
  if (Object.values(conn.suit).find((room) => room.id.startsWith('suit') && [room.p, room.p2].includes(userToChallenge))) throw `${e} El usuario aun esta en una partida, espera a que termine para jugar.`;
  ...
  conn.suit[id] = {
    ...
    status: 'wait',
    waktu: setTimeout(() => {
      if (conn.suit[id]) conn.reply(m.chat, `${e} Tiempo de espera finalizado, el PVP se cancela por falta de respuesta.`, m);
      delete conn.suit[id];
    }, timeout),

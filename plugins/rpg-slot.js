...
const handler = async (m, { args, usedPrefix, command, conn }) => {
  const fa = `${e} Por favor, ingresa la cantidad que desea apostar.`.trim();
  if (!args[0] || isNaN(args[0]) || parseInt(args[0]) <= 0) throw fa;
  
  const apuesta = parseInt(args[0]);
  const users = global.db.data.users[m.sender];
  const time = users.lastslot + 10000;
  if (new Date() - users.lastslot < 10000) throw `${e} Debes esperar ${msToTime(time - new Date())} para usar #slot nuevamente.`;
  if (apuesta < 100) throw `${e} El minimo para apostar es de 100 XP.`;
  if (users.exp < apuesta) {
    throw `${e} Tu XP no es suficiente para aportar esa cantidad.`;
  }

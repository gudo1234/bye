...
text = m.quoted.text;
} else return conn.reply(m.chat, `${e} Te Faltó El Texto!`, m, rcanal);
if (!text) return conn.reply(m.chat, `${e} Te Faltó El Texto!`, m, rcanal);
const who = m.mentionedJid && m.mentionedJid[0] ? m.mentionedJid[0] : m.fromMe ? conn.user.jid : m.sender; 
const mentionRegex = new RegExp(`@${who.split('@')[0].replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*`, 'g');
const mishi = text.replace(mentionRegex, '');
if (mishi.length > 40) return conn.reply(m.chat, `${e} El texto no puede tener mas de 30 caracteres`, m, rcanal);

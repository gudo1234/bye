...
if (!db.data.chats[m.chat].nsfw && m.isGroup) {
    return m.reply(`${e} El contenido *NSFW* está desactivado en este grupo.\n> Un administrador puede activarlo con el comando » *#nsfw*`);
      ...
      await conn.reply(m.chat, `${e} Por favor, ingresa un tag para realizar la búsqueda.`, m, fake);
        return;
    }
    const tag = args[0];
    ...
    try {
        ...
        if (!data || data.length === 0) {
            await conn.reply(m.chat, `${e} No hubo resultados para *${tag}*`, m, fake);
            return;
        }
        const randomIndex = Math.floor(Math.random() * data.length);
        const randomImage = data[randomIndex];
        const imageUrl = randomImage.file_url;
        await conn.sendMessage(m.chat, { image: { url: imageUrl }, caption: `${e} *Resultados para* » ${tag}`, mentions: [m.sender] });
    } catch (error) {
        console.error(error);
        await m.reply(`${e} Ocurrió un error inesperado.`);
    }
};

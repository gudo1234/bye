...
if (!db.data.chats[m.chat].nsfw && m.isGroup) {
    ...
    if (!args[0]) {
        return conn.reply(m.chat, `${e} Por favor, envia un link de Xvideos para descargar el video.`, m);
    }

    try {
        conn.reply(m.chat, `${e} El vídeo está siendo procesado, espere un momento...\n\n- El tiempo de envio depende del peso y duración del video.`, m);

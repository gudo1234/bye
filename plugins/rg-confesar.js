let handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    if (!jid || !pesan) throw m.reply(`${e} *Ejemplo:*\n\n${usedPrefix + command} numero mensaje\n\n*Uso:* ${usedPrefix + command} ${m.sender.split`@`[0]} Hola.`);
    
    jid = jid.replace(/[^0-9]/g, '') + '@s.whatsapp.net'; 
    let data = (await conn.onWhatsApp(jid))[0] || {}; 
    if (!data.exists) throw m.reply(`${e} El número no está registrado en WhatsApp.`);
    if (jid == m.sender) throw m.reply(`${e} No puedes mandarte un mensaje a ti mismo.`);

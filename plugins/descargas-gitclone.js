...
let handler = async (m, { args, usedPrefix, command }) => {
  if (!args[0]) {
    return conn.reply(m.chat, `${e} Por favor, ingresa la URL de un repositorio de GitHub que deseas descargar.`, m, rcanal)
  }
  if (!regex.test(args[0])) {
    return conn.reply(m.chat, `${e} Verifica que la *URL* sea de GitHub`, m, rcanal).then(_ => m.react(error))
  }

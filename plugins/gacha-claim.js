...
async function saveCharacters(characters) {
    try {
        ...
        throw new Error(`${e} No se pudo guardar el archivo characters.json.`);
    }
}
...
let handler = async (m, { conn }) => {
    ...
    if (m.quoted && m.quoted.sender === conn.user.jid) {
        try {
            ...
            await conn.reply(m.chat, `${e} Has reclamado a *${character.name}* con éxito.`, m);
            cooldowns[userId] = now + 30 * 60 * 1000;

        } catch (error) {
            await conn.reply(m.chat, `${e} Error al reclamar el personaje: ${error.message}`, m);
        }

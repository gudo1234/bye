async function handler(m, { conn, args, usedPrefix, command }) {
  ...
  if (!args[0] || !args[1]) {
    const helpMessage = `${e} Debes mencionar a quien quieras regalar *${moneda}*.\n> Ejemplo » *${usedPrefix + command} 25000 @mencion*`.trim();
  ...
  if (!who) return conn.sendMessage(m.chat, {text: `${e} Debes regalar al menos 100 ${moneda}*`, mentions: [m.sender]}, {quoted: m});
  if (!(who in global.db.data.users)) return conn.sendMessage(m.chat, {text: `${e} El usuario ${who} no está en la base de datos.`, mentions: [m.sender]}, {quoted: m});
  if (user[bankType] * 1 < count) return conn.sendMessage(m.chat, {text: `${e} No tienes suficientes ${moneda} en el banco para transferir.`, mentions: [m.sender]}, {quoted: m});

let handler = async (m, { conn, text }) => {
  ...
  if (!text) return conn.reply(m.chat, `${e} *Ingresa el tipo de membresía y la duración.*\n> Ejemplo: #comprarpremium 1 dia`, m);

  let [amount, unit] = text.split(' ');
  amount = parseInt(amount);
  if (isNaN(amount)) return conn.reply(m.chat, `${e} La cantidad debe ser un número.`, m);
  
  const units = { minuto: 1, minutos: 1, hora: 60, horas: 60, dia: 1440, dias: 1440 };
  if (!units[unit.toLowerCase()]) return conn.reply(m.chat, `${e} Unidad de tiempo no válida. Usa minutos, horas o días.`, m);

  let cost = amount * (units[unit.toLowerCase()] / 200);
  if (user.coin < cost) return conn.reply(m.chat, `${e} No tienes suficientes ${global.moneda}. Necesitas ${cost} ${global.moneda} para comprar esta membresía.`, m);

...
export async function before(m, {conn, isAdmin, isBotAdmin, usedPrefix}) {
  ...
  if (chat.antiTraba && m.text.length > 5000) { 
    if (isAdmin) return conn.sendMessage(m.chat, {text: `${e} El administrador @${m.sender.split('@')[0]} acaba de enviar un texto que contiene muchos caracteres -.-!`, mentions: [m.sender]}, {quoted: fakemek});

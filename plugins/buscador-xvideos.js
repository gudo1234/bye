...
const handler = async (m, { conn, args, command, usedPrefix }) => {
    ...
    if (!args[0]) {
        return conn.reply(m.chat, `${e} Por favor, ingresé la búsqueda que desea realizar en Xvideos.\nEjemplo: ${usedPrefix + command} perro.`, m);
    }

    try {
        const results = await xvideosSearch(args.join(' '));
        if (results.length === 0) {
            return conn.reply(m.chat, `${e} No se encontraron resultados para: *${args.join(' ')}*`, m);
        }

        let responseMessage = `${e} *Resultados de búsqueda para:* *${args.join(' ')}*\n\n`;
        ...
        console.error(e);
        return conn.reply(m.chat, `${e} Ocurrió un error al buscar videos. Por favor, intenta de nuevo más tarde.`, m);
    }
};

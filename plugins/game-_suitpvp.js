  ...
  if (room) {
    ...
    if (m.sender == room.p2 && /^(acc(ept)?|terima|aceptar|gas|aceptare?|nao|gamau|rechazar|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      if (/^(tolak|gamau|rechazar|ga(k.)?bisa)/i.test(m.text)) {
        const textno = `${e} @${room.p2.split`@`[0]} Rechazo el PVP, el juego se cancela.`;
      ...
      const comienzop = `${e} Por favor, seleccione una de las siguientes opciones:
piedra
papel
tijera\nGanador +${room.poin}XP\nPerdedor ${room.poin_lose}XP\n*responda al mensaje con la opción que desea*
*ejemplo: papel*`;
      const comienzop2 = `${e} Por favor, seleccione una de las siguientes opciones:
piedra
papel
tijera\nGanador +${room.poin}XP\nPerdedor ${room.poin_lose}XP\n*responda al mensaje con la opción que desea*
*ejemplo: papel*`;
      ...
      room.waktu_milih = setTimeout(() => {
        const iniciativa = `${e} Ningún jugador tomo la iniciativa de empezar el juego, el PVP de ha cancelado.`;
        if (!room.pilih && !room.pilih2) this.sendMessage(m.chat, {text: iniciativa}, {quoted: m});
        else if (!room.pilih || !room.pilih2) {
          win = !room.pilih ? room.p2 : room.p;
          const textnull = `${e} @${(room.pilih ? room.p2 : room.p).split`@`[0]} No elegiste ninguna opción, fin del PVP.`;
    ...
    if (jwb && reg.test(m.text) && !room.pilih && !m.isGroup) {
      ...
      room.text = m.text;
      m.reply(`${e} Has elegido ${m.text}, regresa al grupo y ${room.pilih2 ? `Revisa los resultados` : 'Espera los resultados'}`);
      if (!room.pilih2) this.reply(room.p2, `${e} El oponente a elegido, es tu turno de elegir!!.`, 0);
    }
    if (jwb2 && reg.test(m.text) && !room.pilih2 && !m.isGroup) {
      room.pilih2 = reg.exec(m.text.toLowerCase())[0];
      room.text2 = m.text;
      m.reply(`${e} Has elegido ${m.text}, regresa al grupo y ${room.pilih ? `Revisa los resultados` : 'Espera los resultados'}`);
      if (!room.pilih) this.reply(room.p, `${e} El oponente a elegido, es tu turno de elegir!!.`, 0);
    }
